...
const CtaEvolutionTable = ({
  ...
  h2Added: string[];
  h2Removed: string[];
  /** 'cta' = CTA block only · 'headings' = heading block only · 'both' = both (default) */
  mode?: "cta" | "headings" | "both";
}) => (
  <section
    id={mode === "headings" ? "headings" : "cta-evolution"}
  ...
  >
        ...
        <div className="overflow-hidden rounded-[20px] border border-[#11111f]/10">
          <div className="grid md:grid-cols-2">
            <div className="border-b border-[#11111f]/08 p-5 md:border-b-0 md:border-r">
              ...
              {ctaAdded.length === 0 ? (
                <p className="text-[13px] italic text-[#9b95a8]">None in this period</p>
              ) : (
    ...
    {(mode === "headings" || mode === "both") && (
      <div
        id={mode === "both" ? "headings" : undefined}
        className={mode === "both" ? "scroll-mt-28" : undefined}
      >
        <span className="mb-3 inline-block text-[11px] font-black uppercase tracking-[0.09em] text-primary">
                ...
                <ul className="m-0 list-none space-y-2 p-0">
                  {h2Added.map((h2) => (
                    <li key={h2} className="flex items-start gap-2">
                      <span className="mt-1 flex-none text-[10px] font-black text-[#2e6b44]">
                        +
                      </span>
                      <span className="text-[14px] leading-[1.5] text-[#3f394b]">
                        {h2}
                      </span>
                    </li>
                  ))}
            ...
            <div className="p-5">
              ...
              {h2Removed.length === 0 ? (
                ...
                <ul className="m-0 list-none space-y-2 p-0">
                  {h2Removed.map((h2) => (
                    <li key={h2} className="flex items-start gap-2">
                      <span className="mt-1 flex-none text-[10px] font-black text-[#9b3b2a]">
                        −
                      </span>
                      <span className="text-[14px] leading-[1.5] text-[#3f394b]">
                        {h2}
